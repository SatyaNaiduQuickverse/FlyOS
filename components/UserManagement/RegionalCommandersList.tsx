// components/UserManagement/RegionalCommandersList.tsx - Regional Commanders List Component
import React, { useState } from 'react';
import { 
  Search, UserPlus, Users, Shield, Edit, Trash2, 
  ChevronDown, ChevronUp, Plane as Drone 
} from 'lucide-react';
import { UserRole } from '../../types/auth';
import { UserListProps, UserManagementTab } from './types';

const RegionalCommandersList: React.FC<UserListProps> = ({
  users,
  regions,
  drones,
  searchQuery,
    ...
    <div className="p-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className="relative flex-grow">
          ...
          <input
            type="text"
            placeholder="Search regional commanders by name, username, or email..."
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-4 py-3 bg-gradient-to-r from-gray-800 to-gray-900 rounded-lg border border-gray-700 backdrop-blur-sm">
            ...
            />
            <label htmlFor="show-inactive-commanders" className="text-sm text-gray-300 tracking-wider">
              Show Inactive
            </label>
          </div>
          <button
          ...
          >
            ...
            <span className="tracking-wider font-light">Add Commander</span>
          </button>
        </div>
      </div>
      ...
      {filteredUsers.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-gray-400">
          <div className="relative mb-6">
            ...
            <Shield className="h-16 w-16 text-gray-500 relative" />
          </div>
          <p className="text-xl tracking-wider font-light mb-2">No regional commanders found</p>
          <p className="text-sm text-gray-500">Try adjusting your search or create a new commander</p>
        </div>
      ) : (
            ...
            >
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-gray-800/50 rounded-lg p-3 border border-gray-700/50">
                    <div className="text-xs text-gray-400 mb-1 tracking-wider">OPERATORS</div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700/50">
                      ...
                      {!user.assignedOperators || user.assignedOperators.length === 0 ? (
                        ...
                        <div className="space-y-2 max-h-32 overflow-y-auto">
                          {user.assignedOperators.slice(0, 3).map(operatorId => {
                            const operator = users.find(u => u.id === operatorId);
                            return operator ? (
                              <div key={operatorId} className="flex items-center justify-between bg-gray-700/50 rounded p-2 text-sm">
                                <span>{operator.fullName}</span>
                                <span className="text-xs text-gray-400">@{operator.username}</span>
                              </div>
                            ) : null;
                          })}
                          {user.assignedOperators.length > 3 && (
                            <div className="text-xs text-gray-400 text-center py-1">
                              +{user.assignedOperators.length - 3} more
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700/50">
                      ...
                      {!user.assignedDrones || user.assignedDrones.length === 0 ? (
                        ...
                        <div className="space-y-2 max-h-32 overflow-y-auto">
                          {user.assignedDrones.slice(0, 3).map(droneId => {
                            const drone = drones.find(d => d.id === droneId);
                            return drone ? (
                              <div key={droneId} className="flex items-center justify-between bg-gray-700/50 rounded p-2 text-sm">
                                <span>{drone.id}</span>
                          ...
                          {user.assignedDrones.length > 3 && (
                            <div className="text-xs text-gray-400 text-center py-1">
                              +{user.assignedDrones.length - 3} more
                            </div>
                          )}
                        </div>
                      )}
...
export default RegionalCommandersList;
